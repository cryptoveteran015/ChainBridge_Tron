...
func ToHexWithout0x(b []byte) string {
	hex := Bytes2Hex(b)
	if len(hex) == 0 {
		hex = "0"
	}
...
func MatchHex(hexString1 string, hexString2 string) bool {
	var s1, s2 string
	if Has0xPrefix(hexString1) {
		s1 = hexString1[2:]
	}
	if Has0xPrefix(hexString2) {
		s2 = hexString2[2:]
	}
	intVal1, _ := new(big.Int).SetString(s1, 16)
	intVal2, _ := new(big.Int).SetString(s2, 16)

	// Compare the integer values
	if intVal1.Cmp(intVal2) == 0 {
		return true
	}
